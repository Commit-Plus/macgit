...
enum RepositoryAIPrompt {
    ...
    static func agentPrompt(for request: RepositoryAIAgentRequest) -> String {
        ...
        if request.previousToolResults.isEmpty {
            priorResults = "No Git commands have run yet. Choose an available matching guided action or semantic mutation workflow, or call execute_git before answering a direct question."
        } else {
