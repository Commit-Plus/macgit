...
final class SubtreeSidebarPolicyTests: XCTestCase {
    ...
    func testMissingFolderKeepsMetadataActionsOnly() {
        XCTAssertEqual(
            SubtreeSidebarPolicy.actions(for: entry(folderExists: false)),
            [.editLink, .unlink]
        )
    }

    private func entry(folderExists: Bool) -> GitSubtreeEntry {
        GitSubtreeEntry(
