...
2. Register an Apple app with bundle ID `dev.thanhtran.commitplus`.
...
2. Create or verify an iOS OAuth client with bundle ID `dev.thanhtran.commitplus`.
...
The `BUNDLE_ID` value must equal `dev.thanhtran.commitplus` and `IS_SIGNIN_ENABLED` must be `true`.
