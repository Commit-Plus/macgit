...
- Merged to `main` at `37aaad7`. Per the user's merge-only request, no test or build was rerun on the merged checkout.
...
- [x] Merged to `main` at `37aaad7` and marked Phase 1 `[completed]` in the roadmap. No post-merge test or build was rerun per the user's explicit request.
